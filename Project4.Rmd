...
Model assumptions: (1) The outcome is a binary or dichotomous variable like yes vs no, positive vs negative, 1 vs 0; (2) There is no influential values (extreme values or outliers) in the continuous predictors; (3) There is no high intercorrelations (i.e. multicollinearity) among the predictors.
...
variance of the model, without increasing the bias. In R, the main implementation of random forest is found in the randomForest library [randomForest].
